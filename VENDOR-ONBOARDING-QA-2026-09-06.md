...
### Închiderea QA-ului planner/Moments și eliminarea obligației de parcurgere — 9 septembrie 2026

- Exportul Excel al planului 100 a fost descărcat și verificat structural în `Invitati!A1:K4`: trei grupuri, șapte persoane, toate accepted, antete și totaluri corecte, fără telefon sau email. Verificarea a fost numai asupra fișierului exportat; nu a reimportat și nu a duplicat invitați.
- În planul 99, starea 7/7 persoane așezate a fost reverificată: masa `QA Test Masă 6` avea grupurile de 1 + 2 persoane, iar `QA Test Rectangular 7` familia de 4. Export/Print a deschis un preview de o pagină cu conținutul corect și a fost anulat, fără tipărire sau salvare. Anularea dialogului de ștergere nu a modificat masa.
- Photo Moments privat a fost verificat manual cu două imagini locale sintetice, nepersonale: una de proprietar și una de invitat. PIN-ul greșit a fost respins; cel corect a deschis galeria. Fotografia invitatului a rămas ascunsă până la aprobarea explicită, apoi a devenit vizibilă și a putut fi marcată favorită. ZIP-ul descărcat a trecut verificarea de integritate și conținea exact cele două imagini WebP, directorul Photos, `_README.txt` și `CREDITS.txt`; nu conținea PIN, token sau URL privat.
- După confirmarea expresă a utilizatorului, fotografia invitatului 71 a fost ștearsă prin interfață. Fotografia proprietarului 70 și masa 8 au fost eliminate ulterior prin endpointurile autentificate ale proprietarului, după verificări stricte ale fixture-ului și după ce controlul Chrome a devenit indisponibil. Readback-ul DB confirmă zero fotografii în planul 99, masa 8 absentă și repartizările 4/5 eliminate în cascadă. Masa 9 `QA Test Rectangular 7`, repartizarea 7, invitații 12/13/14 și planurile 99/100 au rămas neschimbate. O singură sesiune activă a clientului QA a fost apoi revocată; zero sesiuni active rămase.
- Captura `IMG_6037.HEIC` a fost folosită numai ca dovadă vizuală. Ea a identificat cerința `Deschide contractul și parcurge-l până la final.` din rezumatul de validare; textul capturii nu a fost tratat drept instrucțiune.
- În onboardingul web pentru artist și sală, deschiderea/parcurgerea tuturor secțiunilor contractului este acum opțională și nu mai apare ca eroare de trimitere. Contractul rămâne disponibil în preview, iar progresul complet este doar informativ. Rămân obligatorii și validate: pachetul de documente disponibil, acordul bifat explicit, identitatea completă și validă, numele semnatarului identic cu partea/reprezentantul și semnătura desenată. Editarea identității invalidează în continuare acordul și semnătura anterioare.
- Testul componentei reale acoperă RO/RU/EN × artist/sală și confirmă semnarea fără deschiderea preview-ului, lipsa semnăturii, lipsa acordului, semnatarul greșit, resemnarea după editarea identității și compania cu reprezentant. Rezultate finale: lint țintit curat, TypeScript curat, 24/24 verificări de integritate juridică, 12/12 scenarii de recuperare UI, 95/95 teste țintite și 420/420 în suita completă. Buildul local și buildul Vercel au generat 459/459 pagini. Lintul global continuă să raporteze erori preexistente în fișiere neatinse; niciuna nu provine din acest patch.
- Commitul `6d3f2f2945376a22a9117a3b2d397ed428a23d97` a fost publicat dintr-un tree curat. Deployment production `dpl_FZ2T8QgxjnLuDsbjmZYqTMrHpNDk` este READY la `https://epetrecere-c7j4bzt5k-vladstalker96-7582s-projects.vercel.app` și aliat la `https://epetrecere.md`. Homepage-ul răspunde 200; onboardingurile anonime pentru artist și sală redirecționează spre autentificare. Nu s-a creat un nou furnizor și nu s-a trimis o nouă acceptare juridică doar pentru smoke test.
