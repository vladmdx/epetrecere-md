...
export default function MessagesPage() {
  ...
  async function sendMessage() {
    ...
    if (!res.ok) { toast.error(t("cabinet.messages.sendError")); setSending(false); return; }
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-8rem)]">
        <div className="flex items-center gap-3 mb-4">
          <Button variant="ghost" size="icon" aria-label={t("cabinet.messages.backToList")} onClick={() => setSelectedConv(null)} className="h-8 w-8">
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <h2 className="font-heading text-lg font-bold">{conv?.artistName || t("cabinet.messages.conversation")}</h2>
        </div>

        <div className="flex-1 overflow-y-auto space-y-2 rounded-xl border border-border/30 bg-accent/10 p-4">
          {messages.length === 0 ? (
            <p className="text-center text-muted-foreground text-sm py-8">{t("cabinet.messages.empty")}</p>
          ) : messages.map(m => (
  ...
  return (
    <div>
      <h1 className="font-heading text-2xl font-bold mb-1">{t("dashboard.messages")}</h1>
      <p className="text-sm text-muted-foreground mb-6">{t("cabinet.messages.subtitle")}</p>

      {conversations.length === 0 ? (
        <p className="py-12 text-center text-muted-foreground">{t("cabinet.messages.noConversations")}</p>
      ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className={cn("flex-1 min-w-0", c.clientUnread === 0 && "ml-5")}>
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-medium text-sm">{c.artistName || t("cabinet.messages.artistFallback")}</p>
                      {c.lastMessageAt && (
