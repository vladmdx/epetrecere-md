...
import { VenueAnalyticsClient } from "../../../sala/analitice/client";
...
export default async function VenueAnalyticsPage({
  ...
  if (!clerkId) redirect(`${localizePath("/sign-in", locale)}?redirect_url=${encodeURIComponent(localizePath(`${venueDashboardBase(scoped.id)}/analitice`, locale))}`);
  ...
  return (
    <VenueAnalyticsClient
      ...
      basePath={`${venueDashboardBase(scoped.id)}/analitice`}
    />
  );
}
