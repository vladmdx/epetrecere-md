...
//   - Authenticated dashboard link only, never private thumbnails or PIN links
//
...
export async function POST(
  ...
  for (const p of approved) {
    ...
    const h = Number(new Intl.DateTimeFormat("en-GB", { timeZone: "Europe/Chisinau", hour: "2-digit", hourCycle: "h23" }).format(new Date(p.createdAt)));
  ...
  // This link grants no access by itself; the dashboard requires sign-in.
  const baseUrl = new URL(process.env.NEXT_PUBLIC_SITE_URL || "https://epetrecere.md").origin;
  ...
  const html = `<!doctype html><html><body style="margin:0;padding:0;background:#0D0D0D;font-family:-apple-system,BlinkMacSystemFont,sans-serif;color:#FAF8F2;">
    <div style="max-width:540px;margin:0 auto;padding:32px 20px;">
      ...
      <h1 style="font-size:28px;font-weight:700;margin:0 0 4px;color:#FAF8F2;">${escapeHtml(plan.title)}</h1>
      ${dateLabel ? `<p style="font-size:13px;color:#B0B0C0;margin:0 0 24px;">${escapeHtml(dateLabel)}</p>` : ""}
      ...
      <div style="display:flex;gap:8px;margin-bottom:24px;flex-wrap:wrap;">
        ...
        <div style="flex:1;min-width:120px;background:#1A1A2E;border:1px solid #C9A84C33;border-radius:12px;padding:12px;">
          ...
          <p style="font-size:24px;font-weight:700;color:#FAF8F2;margin:4px 0 0;">${escapeHtml(peakHour)}</p>
        </div>
      </div>
      ...
      <p style="font-size:11px;color:#666;margin:24px 0 0;line-height:1.5;">
        ...
        Fotografiile pot fi accesate după autentificare în contul tău.
        Descarcă ZIP-ul sau pregătește colajul / albumul din galerie.
      </p>
    </div>
  </body></html>`;
  ...
  try {
    await sendEmail({
      to: plan.ownerEmail,
      subject: `📸 Recap Photo Moments - ${plan.title.replace(/[\r\n]/g, " ")}`,
      html,
    });
    return NextResponse.json({
      sent: true,
    ...
    console.error("[moments-recap] send failed");
    return NextResponse.json(
