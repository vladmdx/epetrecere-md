...
export async function GET(req: NextRequest) {
  ...
  const rows = type === "artist"
    ? await db.select({slug: artists.slug}).from(artists).where(and(eq(artists.isActive,true),inArray(artists.slug,slugs)))
    : await db.select({slug: venues.slug}).from(venues).where(and(publishedVenuePredicateSql(),inArray(venues.slug,slugs)));
