...
export function bookingRequestNewEmail({
  ...
  // Every field here is user/vendor-supplied — escape everything.
  const v = escapeHtml(vendorName);
  const c = escapeHtml(clientName);
  const et = eventType ? escapeHtml(eventType) : null;
  const ed = eventDate ? escapeHtml(eventDate) : null;
  const st = startTime ? escapeHtml(startTime) : null;
  const en = endTime ? escapeHtml(endTime) : null;
  const msg = message ? escapeHtml(message) : null;
  return `
<!DOCTYPE html>
<html>
  ...
  <div style="max-width:600px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="background:#1A1A2E;border-radius:12px;padding:32px;border:1px solid rgba(201,168,76,0.15);">
      ...
      <p style="color:#A0A0B0;font-size:15px;line-height:1.6;margin:0 0 16px;">
        Salut <strong style="color:#FAF8F2;">${v}</strong>,
      </p>
      <p style="color:#A0A0B0;font-size:15px;line-height:1.6;margin:0 0 24px;">
        Ai primit o cerere de rezervare de la <strong style="color:#C9A84C;">${c}</strong>.
      </p>
      <div style="background:rgba(201,168,76,0.08);border-radius:8px;padding:16px;margin:0 0 24px;border-left:3px solid #C9A84C;">
        ${et ? `<p style="color:#D4D4E0;font-size:14px;margin:0 0 8px;"><strong>Eveniment:</strong> ${et}</p>` : ""}
        ${ed ? `<p style="color:#D4D4E0;font-size:14px;margin:0 0 8px;"><strong>Data:</strong> ${ed}</p>` : ""}
        ${st ? `<p style="color:#D4D4E0;font-size:14px;margin:0 0 8px;"><strong>Ora:</strong> ${st}${en ? ` – ${en}` : ""}</p>` : ""}
        ${msg ? `<p style="color:#D4D4E0;font-size:14px;margin:0;"><strong>Mesaj:</strong> ${msg}</p>` : ""}
      </div>
      <p style="color:#A0A0B0;font-size:15px;line-height:1.6;margin:0 0 24px;">
