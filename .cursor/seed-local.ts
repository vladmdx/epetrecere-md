/**
 * Local development seed for Cloud Agent environments.
 *
 * The repo's `scripts/seed.ts` talks to Neon over its HTTP driver, which does
 * not exist against a local PostgreSQL. This variant reuses the app's own
 * `db` handle (postgres.js) so it works against the local cluster created by
 * `.cursor/install.sh`. Reads DATABASE_URL from the environment.
 */
...
async function seed() {
  ...
  for (const section of sections) {
    await db.insert(schema.homepageSections).values({ ...section, isVisible: true });
  }
