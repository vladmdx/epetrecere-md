...
test("both sidebar layouts fail closed and preserve localized profile links on desktop and mobile", () => {
  ...
  const legacyLayout = readFileSync("src/app/[locale]/(vendor)/dashboard/sala/layout.tsx", "utf8");
  const venueLayout = readFileSync("src/app/[locale]/(vendor)/dashboard/locatii/[venueId]/layout.tsx", "utf8");
  const dashboardScope = readFileSync("src/lib/venues/dashboard-scope.ts", "utf8");
  assert.match(legacyLayout, /import \{ redirectLegacySalaPath \} from "@\/lib\/venues\/dashboard-scope"/);
  assert.match(legacyLayout, /await redirectLegacySalaPath\(\{/);
  assert.match(venueLayout, /const venue = await requireLocatieVenue\(venueId, locale\)/);
  assert.match(venueLayout, /isActive=\{venue\.isActive\}/);
  assert.match(dashboardScope, /resolveSelectedVenue\(/);
  assert.match(dashboardScope, /isActive: venues\.isActive/);
  assert.match(dashboardScope, /\.where\(eq\(venues\.id, selection\.venueId\)\)/);
  assert.doesNotMatch(dashboardScope, /eq\(venues\.userId,/);
});
