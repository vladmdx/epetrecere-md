 ...
 try {
  await drizzle(sql,{schema}).transaction(async transaction=>{
   ...
   for(const kind of ["artist","venue"]) {
    ...
    response=await route.POST(req("/api/auth/register-"+kind,{...payloads[kind],...(kind==="artist"?{imageUrl:"not-a-url"}:{capacityMin:200,capacityMax:10})}));
    assert.equal(response.status,400); ok(kind+": invalid profile can fail safely after legal signing");
    response=await accept.GET(); assert.equal(response.status,200);
    assert.equal(response.headers.get("Cache-Control"),"private, no-store");
    const recovery=(await response.json()).onboarding[kind];
    assert.equal(recovery.status,"resumable"); assert.equal(recovery.agreement.locale,"ro");
    assert.equal(recovery.agreement.identity.legalName,body.identity.legalName);
    assert.equal(recovery.agreement.identity.idNumber,body.identity.idNumber);
    assert.equal(recovery.agreement.documents.length,body.documents.length);
    assert.ok(recovery.agreement.documents.every(d=>d.copyUrl===`/api/legal/accept/${d.id}/copy`));
    ok(kind+": authenticated recovery returns frozen party, signing language and current complete copy links");
   ...
   for(const kind of ["artist","venue"]) {
    ...
    const {commissionDueDate,PAYMENT_TERM_DAYS}=require("../src/lib/commissions/service");
    const [confirmed]=await txDb.select().from(schema.bookingRequests).where(eq(schema.bookingRequests.id,booking.id));
    assert.equal(PAYMENT_TERM_DAYS,30); assert.equal(fees[0].dueDate,commissionDueDate(confirmed.confirmedAt));
   ...
   const {commissionDueDate}=require("../src/lib/commissions/service");
   assert.equal(commissionDueDate(new Date("2026-09-03T22:30:00Z")),"2026-10-04");
   assert.equal(commissionDueDate(new Date("2026-02-28T22:30:00Z")),"2026-03-31");
   ok("30-day due date uses Moldova calendar dates, including midnight and DST");
   current=personas.artist;
   const commissionRoute=require("../src/app/api/commissions/route");
   response=await commissionRoute.GET(new server.NextRequest("https://epetrecere.md/api/commissions"));
   let feeBody=await response.json(); assert.equal(response.status,200);
   assert.equal(feeBody.items.length,1); assert.equal(feeBody.totals.pending,22.5);
   ok("vendor commission API totals preserve cents and exclude other providers");
   const {getAdminStats}=require("../src/lib/db/queries/admin-stats");
   const stats=await getAdminStats({from:"2027-09-01",to:"2027-09-01",vendor:"all",categoryId:null,basis:"event"});
   assert.equal(stats.platform.billed,222.5);
   ok("administrator financial aggregates preserve commission cents");
   await txDb.update(schema.bookingRequests).set({status:"accepted"}).where(eq(schema.bookingRequests.id,feeBody.items[0].bookingRequestId));
   response=await commissionRoute.GET(new server.NextRequest("https://epetrecere.md/api/commissions"));
   feeBody=await response.json(); assert.equal(feeBody.items[0].clientEmail,null); assert.equal(feeBody.items[0].clientName,null);
   await txDb.update(schema.bookingRequests).set({status:"confirmed_by_client"}).where(eq(schema.bookingRequests.id,feeBody.items[0].bookingRequestId));
   ok("a legacy premature commission cannot leak unconfirmed client identity");
   ...
   const [otherBooking]=await txDb.insert(schema.bookingRequests).values({venueId:ids.venue,clientUserId:personas.client.id,clientName:"QA Client",clientPhone:"+15555550183",eventType:"other",eventDate:"2027-09-02",guestCount:100,agreedPrice:450,status:"pending",source:"platform"}).returning();
   const otherAction=async action=>bookingRoute.PUT(req("/api/booking-requests/"+otherBooking.id,{action,agreedPrice:450}),{params:Promise.resolve({id:String(otherBooking.id)})});
   current=personas.venue; response=await otherAction("accept"); assert.equal(response.status,200);
   current=personas.client; response=await otherAction("client_confirm"); assert.equal(response.status,200);
   current=personas.venue; response=await otherAction("venue_confirm"); assert.equal(response.status,200,await response.text());
